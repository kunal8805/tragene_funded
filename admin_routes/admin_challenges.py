...
def admin_progression_request_action(request_id, action):
    ...
    if action == 'approve':
        ...
        if progression_request.request_type == 'phase2':
            ...
            message = 'Your request has been approved. Your new trading account is currently being prepared. You will receive your MT5 credentials via your verified email address within 24 hours. Please do not trade on your previous account.'
        else:
            title = 'Funded Request Approved'
            message = 'Your funded account request has been approved. Your funded account credentials will be sent to your verified email address within 24 hours.'
...
def admin_challenges():
    ...
    return render_template('admin/challenge_list.html', challenges=challenges, search_query=search_query, stats=stats)
...
def admin_challenge_detail(challenge_id):
    ...
    return render_template('admin/challenge_detail.html', challenge=challenge)
...
def api_user_calendar(user_id):
    ...
    if not challenge_ids:
        return jsonify({'success': True, 'days': {}, 'summary': {'total_trades': 0, 'total_profit': 0, 'win_rate': 0, 'win_streak': 0, 'best_trade': None, 'top_symbol': None}})
    ...
    for trade in trades:
        ...
        if day_key not in days_data:
            days_data[day_key] = {'total_profit': 0, 'total_trades': 0, 'winning_trades': 0, 'trades': []}
        ...
        days_data[day_key]['trades'].append({
            'id': trade.id, 'symbol': trade.symbol or 'Unknown', 'profit': profit,
    ...
    best_trade_data = {'profit': best_trade.profit or 0, 'symbol': best_trade.symbol or 'Unknown'} if best_trade else None
    ...
    return jsonify({
        'success': True, 'days': days_data,
        'summary': {'total_trades': total_trades, 'total_profit': round(total_profit, 2), 'win_rate': win_rate, 'win_streak': win_streak, 'best_trade': best_trade_data, 'top_symbol': top_symbol}
    })
...
def api_all_challenges():
    ...
    for ch in challenges:
        result.append({
            'id': ch.id, 'user_id': ch.user_id,
            'user_name': ch.user_obj.get_full_name() if ch.user_obj else 'Unknown',
            'user_email': ch.user_obj.email if ch.user_obj else 'Unknown',
            'challenge_name': ch.challenge_template.name if ch.challenge_template else 'Challenge',
            'status': ch.status, 'current_phase': ch.current_phase,
            'profit_percent': ch.profit_percent, 'daily_drawdown': ch.daily_drawdown,
            'overall_drawdown': ch.overall_drawdown, 'risk_score': ch.risk_score,
            'trading_days': ch.trading_days, 'days_remaining': ch.days_remaining,
            'current_balance': ch.current_balance, 'current_equity': ch.current_equity,
            'completed_at': ch.completed_at.isoformat() if ch.completed_at else None,
...
def api_challenge_details(challenge_id):
    ...
    violations = RuleLog.query.filter_by(challenge_id=challenge_id).order_by(RuleLog.created_at.desc()).limit(20).all()
    violations_data = [{'rule_name': v.rule_name, 'severity': v.severity, 'message': v.message, 'current_value': v.current_value, 'threshold_value': v.threshold_value, 'triggered_at': v.created_at.isoformat() if v.created_at else None} for v in violations]
    evidences = ViolationEvidence.query.filter_by(challenge_purchase_id=challenge_id).order_by(ViolationEvidence.created_at.desc()).all()
    evidence_data = [{'id': e.id, 'violation_type': e.violation_type, 'rule_name': e.rule_name, 'rule_limit': e.rule_limit, 'actual_value': e.actual_value, 'drawdown_model': e.drawdown_model, 'day_start_value': e.day_start_value, 'lowest_value': e.lowest_value, 'current_value': e.current_value, 'balance': e.balance, 'equity': e.equity, 'floating_pnl': e.floating_pnl, 'profit_percent': e.profit_percent, 'daily_drawdown': e.daily_drawdown, 'overall_drawdown': e.overall_drawdown, 'trading_days': e.trading_days, 'reason': e.reason, 'severity': e.severity, 'open_positions': e.open_positions_snapshot or [], 'recent_trades': e.recent_trades_snapshot or [], 'account_snapshot': e.account_snapshot_data or {}, 'is_reviewed': e.is_reviewed, 'review_decision': e.review_decision, 'review_notes': e.review_notes, 'violation_timestamp': e.violation_timestamp.isoformat() if e.violation_timestamp else None, 'created_at': e.created_at.isoformat() if e.created_at else None} for e in evidences]
    
    template = ch.challenge_template
    template_rules = None
    if template:
        template_rules = {
            'challenge_type': template.challenge_type or 'one_phase', 'account_size': template.account_size or 10000, 'phase': template.phase or 1,
            'phase1_target': template.phase1_target, 'phase1_daily_loss': template.phase1_daily_loss, 'phase1_daily_dd_type': template.phase1_daily_dd_type or 'equity', 'phase1_overall_loss': template.phase1_overall_loss, 'phase1_overall_dd_type': template.phase1_overall_dd_type or 'equity', 'phase1_min_days': template.phase1_min_days, 'phase1_duration': template.phase1_duration, 'phase1_leverage': template.phase1_leverage or '1:100', 'phase1_rules': template.phase1_rules or '',
            'phase2_target': template.phase2_target, 'phase2_daily_loss': template.phase2_daily_loss, 'phase2_daily_dd_type': template.phase2_daily_dd_type or 'equity', 'phase2_overall_loss': template.phase2_overall_loss, 'phase2_overall_dd_type': template.phase2_overall_dd_type or 'equity', 'phase2_min_days': template.phase2_min_days, 'phase2_duration': template.phase2_duration, 'phase2_leverage': template.phase2_leverage or '1:100', 'phase2_rules': template.phase2_rules or '',
            'instant_daily_loss': template.instant_daily_loss, 'instant_daily_dd_type': template.instant_daily_dd_type or 'equity', 'instant_overall_loss': template.instant_overall_loss, 'instant_overall_dd_type': template.instant_overall_dd_type or 'equity', 'instant_min_days': template.instant_min_days, 'instant_leverage': template.instant_leverage or '1:100', 'instant_rules': template.instant_rules or '',
            'user_profit_share': template.user_profit_share or 80, 'payout_cycle': template.payout_cycle or 'biweekly', 'weekend_trading': template.weekend_trading if template.weekend_trading is not None else True, 'is_active': template.is_active if template.is_active is not None else True, 'description': template.description or '',
            'sl_mandatory_enabled': template.sl_mandatory_enabled if template.sl_mandatory_enabled is not None else False, 'sl_grace_period_minutes': template.sl_grace_period_minutes or 3, 'max_risk_per_trade_percent': template.max_risk_per_trade_percent or 1.5, 'activity_rule_enabled': template.activity_rule_enabled if template.activity_rule_enabled is not None else False, 'max_inactive_days': template.max_inactive_days or 4,
        }
    
    ctype = ch.challenge_type or 'one_phase'
    phase = ch.current_phase or 1
    if ctype == 'instant':
        current_rules = {'phase_name': 'Instant Funded', 'target': template.instant_daily_loss, 'daily_loss': template.instant_daily_loss, 'daily_dd_type': template.instant_daily_dd_type or 'equity', 'overall_loss': template.instant_overall_loss, 'overall_dd_type': template.instant_overall_dd_type or 'equity', 'min_days': template.instant_min_days or 0, 'duration': 365, 'leverage': template.instant_leverage or '1:100'} if template else None
    elif ctype == 'two_phase' and phase == 2:
        current_rules = {'phase_name': 'Phase 2', 'target': template.phase2_target, 'daily_loss': template.phase2_daily_loss, 'daily_dd_type': template.phase2_daily_dd_type or 'equity', 'overall_loss': template.phase2_overall_loss, 'overall_dd_type': template.phase2_overall_dd_type or 'equity', 'min_days': template.phase2_min_days or 0, 'duration': template.phase2_duration or 60, 'leverage': template.phase2_leverage or '1:100'} if template else None
    else:
        current_rules = {'phase_name': 'Phase 1', 'target': template.phase1_target, 'daily_loss': template.phase1_daily_loss, 'daily_dd_type': template.phase1_daily_dd_type or 'equity', 'overall_loss': template.phase1_overall_loss, 'overall_dd_type': template.phase1_overall_dd_type or 'equity', 'min_days': template.phase1_min_days or 0, 'duration': template.phase1_duration or 30, 'leverage': template.phase1_leverage or '1:100'} if template else None

    return jsonify({
        'success': True,
        'challenge': {
            'id': ch.id, 'user_name': ch.user_obj.get_full_name() if ch.user_obj else 'Unknown', 'user_email': ch.user_obj.email if ch.user_obj else 'Unknown',
            'challenge_name': ch.challenge_template.name if ch.challenge_template else 'Challenge', 'status': ch.status, 'monitoring_status': ch.monitoring_status,
            'current_phase': ch.current_phase, 'challenge_type': ctype, 'profit_percent': ch.profit_percent, 'daily_drawdown': ch.daily_drawdown,
            'overall_drawdown': ch.overall_drawdown, 'risk_score': ch.risk_score, 'trading_days': ch.trading_days, 'days_remaining': ch.days_remaining,
            'current_balance': ch.current_balance, 'current_equity': ch.current_equity, 'min_trading_days': ch.challenge_template.phase1_min_days if ch.challenge_template else 5,
            'violation_reason': ch.violation_reason, 'violation_timestamp': ch.violation_timestamp.isoformat() if ch.violation_timestamp else None,
            'review_required': ch.review_required, 'violations': violations_data, 'evidence': evidence_data,
            'template_rules': template_rules, 'current_rules': current_rules,
        }
    })
...
def api_challenge_violations(challenge_id):
    ...
    return jsonify({'success': True, 'violations': [{'id': v.id, 'rule_name': v.rule_name, 'severity': v.severity, 'message': v.message, 'current_value': v.current_value, 'threshold_value': v.threshold_value, 'created_at': v.created_at.isoformat() if v.created_at else None} for v in violations]})
...
def log_rule(challenge_id, rule_name, severity, message, current_value=None, threshold_value=None):
    from models import RuleLog
    try:
        log = RuleLog(challenge_id=challenge_id, rule_name=rule_name, severity=severity, message=message[:500], current_value=current_value, threshold_value=threshold_value, created_at=datetime.now(timezone.utc))
        db.session.add(log)
        db.session.commit()
    except Exception as e:
...
def api_palantir():
    ...
    users_this_month = User.query.filter(extract('month', User.created_at) == now.month, extract('year', User.created_at) == now.year).count()
    yesterday_start = today_start - timedelta(days=1)
    users_yesterday = User.query.filter(User.created_at >= yesterday_start, User.created_at < today_start).count()
    growth_today = round((users_today / max(users_yesterday, 1)) * 100, 1) if users_yesterday > 0 else 100
    
    reg_by_day = db.session.query(func.date(User.created_at).label('date'), func.count(User.id).label('count')).filter(User.created_at >= thirty_days_ago).group_by(func.date(User.created_at)).order_by('date').all()
    ...
    latest_registrations = [{'name': u.get_full_name(), 'email': u.email, 'country': u.country or 'N/A', 'created_at': u.created_at.isoformat() if u.created_at else None} for u in latest_users]
    
    def get_revenue(start=None, end=None):
        q = db.session.query(func.coalesce(func.sum(Payment.amount), 0)).filter(Payment.status.in_(['SUCCESS', 'success']))
    ...
    rev_by_day = db.session.query(func.date(Payment.created_at).label('date'), func.coalesce(func.sum(Payment.amount), 0).label('total')).filter(Payment.status.in_(['SUCCESS', 'success']), Payment.created_at >= thirty_days_ago).group_by(func.date(Payment.created_at)).order_by('date').all()
    rev_labels = [str(r.date) for r in rev_by_day]
    rev_values = [float(r.total) for r in rev_by_day]
    
    active_challenges = ChallengePurchase.query.filter(ChallengePurchase.status.in_(['active', 'funded']), ChallengePurchase.is_terminated == False).count()
    funded_accounts = ChallengePurchase.query.filter(ChallengePurchase.status == 'funded').count()
    breached_accounts = ChallengePurchase.query.filter(ChallengePurchase.is_terminated == True).count()
    
    challenge_rankings = db.session.query(ChallengeTemplate.name, ChallengeTemplate.price, func.count(ChallengePurchase.id).label('total_purchases'), func.coalesce(func.sum(Payment.amount), 0).label('revenue')).join(ChallengePurchase, ChallengePurchase.challenge_template_id == ChallengeTemplate.id).join(Payment, Payment.challenge_purchase_id == ChallengePurchase.id).filter(Payment.status.in_(['SUCCESS', 'success'])).group_by(ChallengeTemplate.id).order_by(func.count(ChallengePurchase.id).desc()).all()
    rankings = [{'name': r.name, 'price': float(r.price), 'total_purchases': r.total_purchases, 'revenue': float(r.revenue)} for r in challenge_rankings]
    ...
    kyc_today = User.query.filter(User.kyc_status == 'submitted', User.kyc_submitted_at >= today_start).count()
    latest_kyc = User.query.filter(User.kyc_status.in_(['submitted', 'approved', 'rejected'])).order_by(User.kyc_submitted_at.desc().nullslast()).limit(10).all()
    kyc_list = [{'user_id': u.id, 'user_name': u.get_full_name(), 'country': u.country or 'N/A', 'status': u.kyc_status, 'submitted_at': u.kyc_submitted_at.isoformat() if u.kyc_submitted_at else None} for u in latest_kyc]
    
    open_tickets = SupportTicket.query.filter_by(status='open').count()
    urgent_tickets = SupportTicket.query.filter(SupportTicket.status == 'open', SupportTicket.priority == 'urgent').count()
    closed_today = SupportTicket.query.filter(SupportTicket.status == 'closed', SupportTicket.resolved_at >= today_start).count()
    latest_tickets = SupportTicket.query.order_by(SupportTicket.created_at.desc()).limit(10).all()
    tickets_list = [{'ticket_number': t.ticket_number, 'user_name': t.user.get_full_name() if t.user else 'Unknown', 'subject': t.subject, 'status': t.status, 'created_at': t.created_at.isoformat() if t.created_at else None} for t in latest_tickets]
    ...
    payments_list = [{'order_id': p.payment_id, 'user_name': p.user.get_full_name() if p.user else 'Unknown', 'amount': float(p.amount), 'status': p.status, 'created_at': p.created_at.isoformat() if p.created_at else None} for p in latest_payments]
    
    active_coupons_count = Coupon.query.filter_by(is_active=True, is_deleted=False).count()
    expired_coupons_count = Coupon.query.filter_by(is_deleted=False).filter(Coupon.expires_at < now).count()
    top_coupons = db.session.query(Coupon.code, func.count(CouponUsage.id).label('uses'), func.coalesce(func.sum(CouponUsage.final_price), 0).label('revenue')).join(CouponUsage, CouponUsage.coupon_id == Coupon.id).filter(Coupon.is_deleted == False).group_by(Coupon.id).order_by(func.count(CouponUsage.id).desc()).limit(10).all()
    coupons_list = [{'code': c.code, 'uses': c.uses, 'revenue': float(c.revenue)} for c in top_coupons]
    
    total_partners = User.query.filter_by(role='partner', is_banned=False).count()
    partner_rankings = db.session.query(User.first_name, User.last_name, func.count(PartnerEarnings.id).label('referrals'), func.coalesce(func.sum(PartnerEarnings.purchase_amount), 0).label('revenue'), func.coalesce(func.sum(PartnerEarnings.partner_share), 0).label('commission')).join(PartnerEarnings, PartnerEarnings.partner_id == User.id).filter(User.role == 'partner').group_by(User.id).order_by(func.count(PartnerEarnings.id).desc()).limit(5).all()
    partners_list = [{'name': f"{p.first_name} {p.last_name}", 'referrals': p.referrals, 'revenue': float(p.revenue), 'commission': float(p.commission)} for p in partner_rankings]
    
    near_daily_loss = ChallengePurchase.query.filter(ChallengePurchase.status.in_(['active', 'funded']), ChallengePurchase.is_terminated == False, ChallengePurchase.daily_drawdown >= 4.0).count()
    near_overall_loss = ChallengePurchase.query.filter(ChallengePurchase.status.in_(['active', 'funded']), ChallengePurchase.is_terminated == False, ChallengePurchase.overall_drawdown >= 8.0).count()
    require_review = ChallengePurchase.query.filter_by(review_required=True).count()
    breached_today = ChallengePurchase.query.filter(ChallengePurchase.is_terminated == True, ChallengePurchase.completed_at >= today_start).count()
    
    summary = {'new_users_today': users_today, 'revenue_today': revenue_today, 'top_challenge': top_challenge, 'pending_kyc': pending_kyc, 'pending_tickets': open_tickets, 'system_health': 'NORMAL'}
    actions = {'pending_kyc': pending_kyc, 'open_tickets': open_tickets, 'failed_payments': failed_payments_count, 'near_breach': near_daily_loss + near_overall_loss, 'require_review': require_review}
    
    return jsonify({
        'success': True, 'summary': summary, 'actions': actions,
        'users': {'total': total_users, 'today': users_today, 'this_month': users_this_month, 'growth_today': growth_today, 'registrations_by_day_labels': reg_labels, 'registrations_by_day_values': reg_values, 'latest': latest_registrations},
        'revenue': {'today': revenue_today, 'yesterday': revenue_yesterday, 'this_month': revenue_this_month, 'lifetime': revenue_lifetime, 'revenue_by_day_labels': rev_labels, 'revenue_by_day_values': rev_values},
        'challenges': {'active': active_challenges, 'funded': funded_accounts, 'breached': breached_accounts, 'rankings': rankings},
        'kyc': {'pending': pending_kyc, 'approved': approved_kyc, 'rejected': rejected_kyc, 'today': kyc_today, 'latest': kyc_list},
        'support': {'open': open_tickets, 'urgent': urgent_tickets, 'closed_today': closed_today, 'latest': tickets_list},
        'payments': {'successful': successful_payments, 'pending': pending_payments_count, 'failed': failed_payments_count, 'success_rate': success_rate, 'latest': payments_list},
        'coupons': {'active': active_coupons_count, 'expired': expired_coupons_count, 'top_10': coupons_list},
        'partners': {'total': total_partners, 'rankings': partners_list},
        'risk': {'near_daily_loss': near_daily_loss, 'near_overall_loss': near_overall_loss, 'require_review': require_review, 'breached_today': breached_today, 'funded_accounts': funded_accounts}
    })
...
def api_palantir_activity():
    ...
    for u in users:
        events.append({'timestamp': u.created_at.isoformat() if u.created_at else None, 'type': 'registration', 'user': u.get_full_name(), 'description': f"New registration from {u.country or 'Unknown'}"})
    purchases = ChallengePurchase.query.order_by(ChallengePurchase.purchase_date.desc()).limit(10).all()
    for p in purchases:
        events.append({'timestamp': p.purchase_date.isoformat() if p.purchase_date else None, 'type': 'challenge_purchase', 'user': p.user.get_full_name() if p.user else 'Unknown', 'description': f"Purchased {p.challenge_template.name if p.challenge_template else 'Challenge'}"})
    payments_list = Payment.query.order_by(Payment.created_at.desc()).limit(10).all()
    for p in payments_list:
        status_lower = (p.status or '').lower()
        event_type = 'payment_success' if 'success' in status_lower else ('payment_failed' if 'fail' in status_lower else 'payment')
        events.append({'timestamp': p.created_at.isoformat() if p.created_at else None, 'type': event_type, 'user': p.user.get_full_name() if p.user else 'Unknown', 'description': f"Payment of ₹{float(p.amount or 0):,.2f} - {p.status}"})
    kyc_users = User.query.filter(User.kyc_status.in_(['submitted', 'approved', 'rejected'])).order_by(User.kyc_submitted_at.desc().nullslast()).limit(10).all()
    for u in kyc_users:
        events.append({'timestamp': u.kyc_submitted_at.isoformat() if u.kyc_submitted_at else None, 'type': f"kyc_{u.kyc_status}", 'user': u.get_full_name(), 'description': f"KYC {u.kyc_status}"})
    tickets = SupportTicket.query.order_by(SupportTicket.created_at.desc()).limit(10).all()
    for t in tickets:
        events.append({'timestamp': t.created_at.isoformat() if t.created_at else None, 'type': 'support_ticket', 'user': t.user.get_full_name() if t.user else 'Unknown', 'description': f"Ticket #{t.ticket_number}: {(t.subject or '')[:50]}"})
    usages = CouponUsage.query.order_by(CouponUsage.used_at.desc()).limit(5).all()
    for u in usages:
        events.append({'timestamp': u.used_at.isoformat() if u.used_at else None, 'type': 'coupon_used', 'user': u.user.get_full_name() if u.user else 'Unknown', 'description': f"Used coupon {u.coupon.code if u.coupon else 'Unknown'}"})
...
def admin_violations():
    ...
    violations = query.order_by(ViolationEvidence.created_at.desc()).paginate(page=page, per_page=20, error_out=False)
    return render_template('admin/violations_list.html', violations=violations, violation_type=violation_type, review_status=review_status)
...
def admin_violation_detail(evidence_id):
    ...
    return render_template('admin/violation_detail.html', evidence=evidence, challenge=challenge)
...
def admin_violation_action(evidence_id):
    ...
    if action == 'confirm_fail':
        ...
        log_rule(challenge.id, "admin_confirmed_fail", "critical", "Admin confirmed violation. Account failed.")
        ...
        evidence.review_notes = notes
        log_rule(challenge.id, "admin_cleared_violation", "info", "Admin cleared violation. Account restored to active.")
        _notify_user(challenge.user_id, "Account Restored", "Your account has been reviewed and restored to active status. You may continue trading.")
...
def api_challenge_violations_list(challenge_id):
    from models import ViolationEvidence
    violations = ViolationEvidence.query.filter_by(challenge_purchase_id=challenge_id).order_by(ViolationEvidence.created_at.desc()).all()
...
# ========================================================================
# MARKETPLACE LOCKDOWN ROUTES
# ========================================================================

@admin_bp.route('/api/marketplace/toggle', methods=['POST'])
@admin_required
def toggle_marketplace_lockdown():
    """Toggle marketplace purchase lockdown ON/OFF"""
    admin_user = User.query.get(session['user_id'])
    settings = SiteSettings.get_settings()
    
    # Toggle the lock
    settings.marketplace_locked = not settings.marketplace_locked
    
    if settings.marketplace_locked:
        # LOCKING the marketplace
        reason = request.form.get('reason', '').strip()
        settings.marketplace_lock_reason = reason or 'Marketplace is temporarily disabled for maintenance'
        settings.marketplace_locked_by = admin_user.id
        settings.marketplace_locked_at = datetime.now(timezone.utc)
        
        log = AdminLog(
            admin_id=admin_user.id,
            action='lock_marketplace',
            target_type='marketplace',
            target_id=settings.id,
            details=f'Marketplace LOCKED by {admin_user.get_full_name()}. Reason: {settings.marketplace_lock_reason}',
            ip_address=request.remote_addr
        )
        db.session.add(log)
        flash('🔒 Marketplace has been LOCKED. Users cannot make purchases.', 'success')
    else:
        # UNLOCKING the marketplace
        settings.marketplace_lock_reason = ''
        settings.marketplace_locked_by = None
        settings.marketplace_locked_at = None
        
        log = AdminLog(
            admin_id=admin_user.id,
            action='unlock_marketplace',
            target_type='marketplace',
            target_id=settings.id,
            details=f'Marketplace UNLOCKED by {admin_user.get_full_name()}',
            ip_address=request.remote_addr
        )
        db.session.add(log)
        flash('🔓 Marketplace has been UNLOCKED. Users can now purchase challenges.', 'success')
    
    db.session.commit()
    return redirect(url_for('admin.admin_dashboard'))


@admin_bp.route('/api/marketplace/status')
@admin_required
def marketplace_status():
    """Get current marketplace lockdown status"""
    settings = SiteSettings.get_settings()
    return jsonify({
        'success': True,
        'locked': settings.marketplace_locked,
        'reason': settings.marketplace_lock_reason,
        'locked_at': settings.marketplace_locked_at.isoformat() if settings.marketplace_locked_at else None,
        'locked_by': settings.locked_by_admin.get_full_name() if settings.locked_by_admin else None
    })